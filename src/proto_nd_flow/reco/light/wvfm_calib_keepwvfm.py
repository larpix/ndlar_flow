...
class WaveformCalib_kpwvfm(H5FlowStage):
    '''
        Preserved single-channel waveforms while applying
    ...
    def __init__(self, **params):
        ...
        self.thresh = params.get('thresh',{'default': 1.0})
        self.thresh_mc = params.get('thresh_mc',{'default': 1.0})

    def _load_thresh_data(self, thresh_data):
        self.thresh = defaultdict(lambda : defaultdict((lambda : thresh_data.get('default',1.0))))
        for adc in thresh_data:
            if adc == 'default':
                continue
            for chan in thresh_data[adc]:
                self.thresh[adc][chan] = thresh_data[adc][chan]
    ...
    def init(self, source_name):
        ...
        if resources['RunData'].is_mc:
            ...
            self._load_thresh_data(self.thresh_mc)
        else:
    ...
    def run(self, source_name, source_slice, cache):
        super(WaveformCalib_kpwvfm, self).run(source_name, source_slice, cache)
